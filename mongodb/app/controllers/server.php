...
class ServerController extends BaseController {
   ...
   return plus;
	...
	public function doReplication() {
		...
		try {
			$ret = $this->_mongo->selectDB("admin")->execute('function () { return db.getReplicationInfo(); }');
		...
		$this->slaves = array();
		try {
			//$query = $this->_mongo->selectDB("admin")->selectCollection("slaves")->find();
			$query = $this->_mongo->selectDB("admin")->execute('function(){return rs.status();}');
		...
		$this->masters = array();
		try {
			$query = $this->_mongo->selectDB("admin")->selectCollection("sources")->find();
			foreach ($query as $one) {
				foreach ($one as $param=>$value) {
		...
		//me
		try {
			$this->me = $this->_mongo->selectDB("admin")->selectCollection("me")->findOne();
		} catch (Exception $e) {
			$this->me = array();
		}
